...
let email = ''
Given(/^the user navigates to the sign-in page$/, function () {
...
When(/^clicks the sign-in button$/, function () {
    cy.get("button#sign-in").click();
});
Then(/^the user is redirected to the adding birthdays page$/, function () {
    // Check that the user is redirected to the add-birthdays page
